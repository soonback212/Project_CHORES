...
NavigateThroughPosesNavigator::onLoop()
{
  ...
  // number of behaviors, and distance remaining to goal, etc)
  ...
  blackboard->get<int>("number_behaviors", recovery_count);
  feedback_msg->number_of_behaviors = recovery_count;
...
NavigateThroughPosesNavigator::initializeGoalPoses(ActionT::Goal::ConstSharedPtr goal)
{
  ...
  auto blackboard = bt_action_server_->getBlackboard();
  blackboard->set<int>("number_behaviors", 0);  // NOLINT
